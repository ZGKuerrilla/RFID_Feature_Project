import express from "express";
import cors from "cors";
import rd2 from "./model/rd2.js";

const app = express();

app.use(cors());
app.use(express.json());

const PORT = process.env.PORT || 3000;

let reader = null;

/**
 * 捕捉未處理例外
 */
process.on("uncaughtException", (err) => {
  console.error("未捕捉的例外:", err);
});

process.on("unhandledRejection", (reason) => {
  console.error("未處理的 Promise 拒絕:", reason);
});

/**
 * Express async error wrapper
 */
function asyncHandler(fn) {
  return (req, res, next) => {
    Promise.resolve(fn(req, res, next)).catch(next);
  };
}

/**
 * 檢查 RFID Reader 是否已開啟
 */
function ensureReaderOpen(res) {
  if (!reader || !reader.port?.isOpen) {
    res.status(400).json({
      success: false,
      message: "尚未開啟串口",
    });

    return false;
  }

  return true;
}

/**
 * 開啟 Serial Port
 */
app.post(
  "/openPort",
  asyncHandler(async (req, res) => {
    const portPath = req.body?.port || "COM4";
    const baudRate = req.body?.baudRate || 57600;

    if (reader?.port?.isOpen) {
      return res.json({
        success: true,
        message: "串口已經開啟",
      });
    }

    reader = new rd2(portPath, baudRate);

    try {
      await reader.open();
    } catch (error) {
      reader = null;
      throw error;
    }

    reader.on("tag", ({ epc }) => {
      console.log(`[標籤事件] EPC: ${epc}`);
    });

    console.log(`串口已成功開啟 (${portPath} @ ${baudRate}bps)`);

    res.json({
      success: true,
      message: "串口已成功開啟",
      port: portPath,
      baudRate,
    });
  }),
);

/**
 * 關閉 Serial Port
 */
app.post(
  "/closePort",
  asyncHandler(async (req, res) => {
    if (!ensureReaderOpen(res)) {
      return;
    }

    const portPath = reader.port.path;

    await reader.close();

    reader = null;

    console.log(`串口已關閉 (${portPath})`);

    res.json({
      success: true,
      message: "串口已關閉",
    });
  }),
);

/**
 * 啟用主動標籤回報
 */
app.post(
  "/enableTagDisplay",
  asyncHandler(async (req, res) => {
    if (!ensureReaderOpen(res)) {
      return;
    }

    reader.enableTagDisplay();

    console.log("主動標籤回報已啟用");

    res.json({
      success: true,
      message: "主動標籤回報已啟用",
    });
  }),
);

/**
 * 停用主動標籤回報
 */
app.post(
  "/disableTagDisplay",
  asyncHandler(async (req, res) => {
    if (!ensureReaderOpen(res)) {
      return;
    }

    reader.disableTagDisplay();

    console.log("主動標籤回報已停用");

    res.json({
      success: true,
      message: "主動標籤回報已停用",
    });
  }),
);

/**
 * 查詢目前功率
 */
app.get(
  "/getPower",
  asyncHandler(async (req, res) => {
    if (!ensureReaderOpen(res)) {
      return;
    }

    const power = await reader.getPower();

    console.log(`目前功率為 ${power} dBm`);

    res.json({
      success: true,
      power,
    });
  }),
);

/**
 * 設定功率
 */
app.post(
  "/setPower",
  asyncHandler(async (req, res) => {
    if (!ensureReaderOpen(res)) {
      return;
    }

    const power = req.body?.power;

    if (typeof power !== "number" || !Number.isInteger(power) || power < 5 || power > 33) {
      return res.status(400).json({
        success: false,
        message: "功率需為 5~33 之間的整數",
      });
    }

    await reader.setPower(power);

    console.log(`功率已設定為 ${power} dBm`);

    res.json({
      success: true,
      message: `已設定為 ${power} dBm`,
      power,
    });
  }),
);

/**
 * 查詢 Reader 基本參數
 */
app.get(
  "/queryBasicParams",
  asyncHandler(async (req, res) => {
    if (!ensureReaderOpen(res)) {
      return;
    }

    const params = await reader.queryBasicParams();
    const power = await reader.getPower();

    console.log("目前基本參數：");
    console.log(`通訊模式：${params.OMDesc}`);
    console.log(`工作模式：${params.WMDesc}`);
    console.log(`標籤讀取類型：${params.RTDesc}`);
    console.log(`數據輸出間隔 RI：${params.RI} * 10 ms`);
    console.log(`單標籤過濾時間 RD：${params.RD} * 0.5 s`);
    console.log(`輸出起始位元組：${params.Offset}`);
    console.log(`蜂鳴器：${params.buzzerDesc}`);
    console.log(`頻率通道：CH ${params.CH} → ${params.rfFreq} (${params.region})`);

    res.json({
      success: true,
      params: {
        ...params,
        power,
      },
    });
  }),
);

/**
 * 設定蜂鳴器
 */
app.post(
  "/setBuzzer",
  asyncHandler(async (req, res) => {
    if (!ensureReaderOpen(res)) {
      return;
    }

    const on = req.body?.on;

    if (typeof on !== "boolean") {
      return res.status(400).json({
        success: false,
        message: "on 必須為 true 或 false",
      });
    }

    await reader.setBuzzer(on);

    console.log(`蜂鳴器已${on ? "開啟" : "關閉"}`);

    res.json({
      success: true,
      message: `蜂鳴器已${on ? "開啟" : "關閉"}`,
    });
  }),
);

/**
 * 設定工作模式
 *
 * 0 = 命令模式
 * 1 = 主動模式
 * 2 = 被動模式
 */
app.post(
  "/setWorkMode",
  asyncHandler(async (req, res) => {
    if (!ensureReaderOpen(res)) {
      return;
    }

    const mode = req.body?.mode;

    const modeMap = {
      0: "命令模式",
      1: "主動模式",
      2: "被動模式",
    };

    if (![0, 1, 2].includes(mode)) {
      return res.status(400).json({
        success: false,
        message: "mode 僅可為 0、1、2",
      });
    }

    await reader.setWorkMode(mode);

    console.log(`工作模式已設定為 ${modeMap[mode]} (${mode})`);

    res.json({
      success: true,
      message: `工作模式已設定為 ${modeMap[mode]}`,
      mode,
    });
  }),
);

/**
 * 恢復出廠設定並重新啟動
 */
app.post(
  "/restoreAndReboot",
  asyncHandler(async (req, res) => {
    if (!ensureReaderOpen(res)) {
      return;
    }

    await reader.restoreFactorySettings();

    console.log("恢復出廠設定完成");

    await reader.rebootDevice();

    console.log("設備已重啟");

    res.json({
      success: true,
      message: "設備恢復出廠設定並已重新啟動",
    });
  }),
);

/**
 * 取得已讀取的 EPC 清單
 */
app.get(
  "/getSeenTags",
  asyncHandler(async (req, res) => {
    if (!ensureReaderOpen(res)) {
      return;
    }

    const tags = reader.getSeenEPCs();

    res.json({
      success: true,
      count: tags.length,
      tags,
    });
  }),
);

/**
 * 清除 EPC 清單
 */
app.post(
  "/clearEPCs",
  asyncHandler(async (req, res) => {
    if (!ensureReaderOpen(res)) {
      return;
    }

    reader.clearSeenEPCs();

    res.json({
      success: true,
      message: "已清除 EPC 清單",
    });
  }),
);

/**
 * 單次讀取 EPC
 */
app.get(
  "/readEPCOnce",
  asyncHandler(async (req, res) => {
    if (!ensureReaderOpen(res)) {
      return;
    }

    const result = await reader.readSingleEPC();

    res.json({
      success: true,
      epc: result.epc,
    });
  }),
);

/**
 * 404
 */
app.use((req, res) => {
  res.status(404).json({
    success: false,
    message: "API 路徑不存在",
  });
});

/**
 * Global Error Handler
 */
app.use((err, req, res, next) => {
  console.error("全局錯誤:", err);

  res.status(500).json({
    success: false,
    message: err.message || "未知錯誤",
  });
});

/**
 * Start Server
 */
app.listen(PORT, () => {
  console.log(`RD2 RFID API Server 已啟動`);
  console.log(`http://localhost:${PORT}`);
});
